  ...
  app.post<{
    ...
    async (request, reply) => {
        ...
        .object({
          ...
          cityContext: z
            .object({
              ...
              // From `CityStore.cityName` -- the user's currently-selected
              // city, not resolved from this POI's own coordinates. Used
              // only to check for a real UNESCO Creative City match (see
              // unesco.ts's own comment on why city-name is the only
              // practical key for that dataset).
              cityName: z.string().trim().max(200).optional(),
      ...
      const [websiteExcerpt, wikivoyageGuide, unescoSite, creativeCity] = await Promise.all([
        ...
        // reliably match with a keyword pattern.
        cityContext?.cityName ? fetchCreativeCity(cityContext.cityName) : Promise.resolve(null),
      ]);
      ...
      // Same "only pay the cost when it's actually relevant" pattern as
      // transit above -- looked up from `local-resources.ts`'s small,
      // hand-verified list (~27 countries; most countries genuinely have no
      // real equivalent, see that file's own comment) rather than trusted
      // to the model's own fuzzy recall, which tested unreliable live.
      const looksLikeEquipmentQuestion =
        /equipment|\bgear\b|\bborrow\b|\brent\b|\blend\b|don'?t have (a|an|any)|ekipman|malzeme|\böd[üu]n[çc]|kirala|utstyr|l[åa]ne|leie/i.test(
          message
        );
      ...
      // Real academic research (OpenAlex, keyless) -- deliberately gated to
      // deep-history-style questions, not fetched on every message: coverage
      // is rare (only well-studied landmarks) and academic abstracts are
      // dense/jargon-heavy, not something worth the round trip for a
      // generic "what is this place" ask that Wikipedia/Wikivoyage already
      // cover better. See academic.ts's own comment on why OpenAlex specifically.
      const looksLikeDeepHistoryQuestion =
      ...
      const transitLine = transitResult
        ...
        : null;
      const localResourceLine = localResource
        ? `Real, verified local resource for borrowing/renting equipment in ${cityContext?.countryName}: ${localResource.organization}${
      ...
      const intangibleHeritageLine = intangibleHeritage.length
        ? `Real UNESCO-recognized intangible cultural heritage from ${cityContext?.countryName} (a country-level list -- not necessarily specific to this place): ${intangibleHeritage
            ...
            .join(' | ')}`
        : null;
      const creativeCityLine = creativeCity
        ? `${cityContext?.cityName} is a real UNESCO Creative City of ${creativeCity.field} (city-level, not necessarily specific to this place). Official UNESCO description: "${creativeCity.description}"`
        : null;
      const academicFindingLine = academicFinding
        ? `Real academic research paper about this place${academicFinding.year ? ` (${academicFinding.year})` : ''}, titled "${academicFinding.title}". Abstract (dense academic language -- paraphrase, don't quote): "${academicFinding.abstract}"`
        : null;
      const placeContext = [
        `Name: ${name}`,
        category ? `Category: ${category}` : null,
...
${
  creativeCityLine
    ? `\nCREATIVE CITY RULE: this city's real UNESCO Creative City designation is given above, real UNESCO text, not generated by you — it's a city-level fact, not necessarily about this specific place. Mention it when genuinely relevant (someone asking what the city/area is known for, its food/design/music scene), don't force it into an unrelated answer, and never claim this designation for a city not given this exact context.\n`
    : ''
}
${
  academicFindingLine
    ? `\nACADEMIC RESEARCH RULE: the abstract above is real text from an actual academic paper, not generated by you, but it's written in dense scholarly language a casual traveler doesn't want verbatim — paraphrase the relevant finding in your own plain, friendly words, never quote the jargon directly or name-drop the paper's own academic terminology. If it doesn't actually answer what was asked, ignore it rather than forcing it in.\n`
    : ''
}
${
  cityContextLine
